...
def rdc_transform(data, k=20, s=1.0/6.0, nl_func=np.sin):
    ...
    mean = np.zeros(k)
    cov = np.diag(np.full(k, s))
    gaussian_samples = [stats.norm.rvs(mean, cov) for i in range(n_features)]
    projected_samples = [s / (n_features + 1) * np.dot(c, w) for c, w in zip(features, gaussian_samples)]
...
def rdc_cca(i, j, features):
    cca = sklearn.cross_decomposition.CCA(n_components=1, max_iter=100)
