...
from spnflow.structure.leaf import Bernoulli, Categorical
from spnflow.learning.wrappers import learn_estimator, learn_classifier
from spnflow.algorithms.inference import log_likelihood, mpe
...
class SPNEstimator(BaseEstimator, DensityMixin):
    ...
    def __init__(
        ...
        self.learn_leaf_params = learn_leaf_params
    ...
    def fit(self, X, y=None):
        ...
        self.spn_ = learn_estimator(
            data=X, distributions=self.distributions, domains=None,
            learn_leaf=self.learn_leaf, learn_leaf_params=self.learn_leaf_params,
...
class SPNClassifier(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y):
        ...
        self.spn_ = learn_classifier(
            data=data, distributions=distributions, domains=None,
            learn_leaf=self.learn_leaf, learn_leaf_params=self.learn_leaf_params,
            split_rows=self.split_rows, split_cols=self.split_cols,
            split_rows_kwargs=self.split_rows_kwargs, split_cols_kwargs=self.split_cols_kwargs,
            min_rows_slice=self.min_rows_slice, min_cols_slice=self.min_cols_slice,
